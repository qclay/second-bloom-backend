  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async handleConnection(client: AuthenticatedSocket) {
    try {
      ...
      const wasOnline = this.conversationService.isUserOnline(userId);
      this.conversationService.setUserOnline(userId);

      this.logger.log(`Client ${client.id} connected as user ${userId}`);

      const conversationIds =
        await this.conversationService.getConversationIds(userId);
      for (const cid of conversationIds) {
        void client.join(`conversation:${cid}`);
      }

      client.emit(CONVERSATION_EVENTS.CONNECTED, {
        userId,
        socketId: client.id,
  ...
  async handleDisconnect(client: AuthenticatedSocket) {
    if (client.userId) {
      ...
      if (!stillOnline) {
        this.conversationService.setUserOffline(client.userId);

        const conversationIds =
          await this.conversationService.getConversationIds(client.userId);
  ...
  handleGetOnlineStatus(
    ...
    const onlineUsers = this.conversationService.getOnlineUserIds();
  ...
  handleTypingStart(
    ...
    client
      .to(`conversation:${data.conversationId}`)
      .emit(CONVERSATION_EVENTS.USER_TYPING, {
  ...
  async handleMarkRead(
    ...
    try {
      ...
      this.server
        .to(`conversation:${data.conversationId}`)
        .emit(CONVERSATION_EVENTS.MESSAGES_READ, {
          conversationId: data.conversationId,
          userId: client.userId,
          count: result.count,
        });
  ...
  async handleDeleteMessage(
    ...
    try {
      ...
      this.server
        ...
        .emit(CONVERSATION_EVENTS.MESSAGE_DELETED, {
  ...
  async handleEditMessage(
    ...
    try {
      ...
      this.server
        .to(`conversation:${message.conversationId}`)
        .emit(CONVERSATION_EVENTS.MESSAGE_EDITED, message);
  ...
  notifyNewMessage(
    ...
    this.sendToUser(recipientId, CONVERSATION_EVENTS.NEW_MESSAGE, message);
    this.server
