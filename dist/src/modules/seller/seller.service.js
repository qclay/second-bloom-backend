...
let SellerService = SellerService_1 = class SellerService {
    ...
    async getActivities(sellerId, page = 1, limit = 20, type) {
        ...
        const orderDtos = orders.map((order) => order_response_dto_1.OrderResponseDto.fromEntity(order));
        const auctionDtos = auctions.map((auction) => auction_response_dto_1.AuctionResponseDto.fromEntity(auction));
        return {
            orders: orderDtos,
